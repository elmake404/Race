...
Shader "CustomMaterialCurvedTextured" {
   Properties {
      ...
      _OffsetX ("_OffsetX", Float) = 1.0
      _OffsetY ("_OffsetY", Float) = 1.0
   }
   SubShader {
    ...
    Pass {   
         ...
         half4 frag(vertexOutput input) : COLOR
         {
            ...
            tempcol = tex2D(_Texture,half2(input.uv.x * _ScaleX + _OffsetX,input.uv.y * _ScaleY + _OffsetY).xy ).rgb * tempcol;

            half4 col = fixed4(tempcol.rgb, 1.0f) ;
            return col;
         }
